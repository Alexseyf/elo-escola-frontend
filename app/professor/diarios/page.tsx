    ...
    <RouteGuard allowedRoles={['PROFESSOR']}>
      <div className="min-h-screen bg-soft-gray pb-12">
        <PageHeader
          ...
          subtitle="Acompanhamento e registro do desenvolvimento dos alunos."
        />
                ...
                <div key={turma.id} className="space-y-4">
                  <div className="flex items-center justify-between px-2">
                    <div className="flex items-center gap-3">
                      ...
                      <h2 className="text-lg font-bold text-gray-900">{formatarNomeTurma(turma.nome)}</h2>
                    </div>
                    <span className="bg-white text-gray-500 px-3 py-1 rounded-full text-xs font-bold border border-gray-100 shadow-sm">
                          ...
                          >
                            ...
                            >
                              <div className="flex items-center justify-between gap-4">
                                <h4 className="font-bold text-gray-900 truncate flex-1">
                                  {aluno.nome.split(' ').slice(0, 2).join(' ')}
                                </h4>

                                {temDiario ? (
                                  <span className="flex items-center gap-1.5 bg-soft-green text-soft-green-foreground px-2.5 py-1 rounded-lg text-[10px] font-black uppercase border border-soft-green-border shrink-0">
                                    <CheckCircle2 className="w-3.5 h-3.5" />
                                    Preenchido
                                  </span>
                                ) : (
                                  <span className="flex items-center gap-1.5 bg-gray-50 text-gray-400 px-2.5 py-1 rounded-lg text-[10px] font-black uppercase border border-gray-100 shrink-0">
                                    Pendente
                                  </span>
                                )}
                              </div>
